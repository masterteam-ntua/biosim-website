---
layout: page
permalink: /publications/
title: Publications
description: BibTeX-driven publication list with year filters.
nav: true
nav_order: 3
---

<div class="publication-tools" data-aos="fade-up">
  <button class="filter-button" type="button">All years</button>
  <button class="filter-button" type="button">2026</button>
  <button class="filter-button" type="button">2025</button>
  <button class="filter-button" type="button">2024</button>
</div>

<div class="publications" data-aos="fade-up">
  {% bibliography %}
</div>
