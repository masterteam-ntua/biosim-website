...
The workflow in `.github/workflows/deploy.yml` builds the site on pushes to `main` or `master` and publishes `_site` to the `gh-pages` branch using `JamesIves/github-pages-deploy-action@v4`. The `biosim-staging` branch keeps the full imported content for review and does not deploy live Pages.
